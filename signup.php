...
if (!empty($_REQUEST['captcha'])) {
	...
	else //submit form from here
	{
		...
		else
		{
			...
			$body .= "We have received your request for the faculty position at National Institute of Technology, Silchar.<br /><br/><br/><b>To verify your account <a href='http://recruitment.nits.ac.in/verify.php?vid=".$verify_id."&rid=".$reg_id."'>CLICK HERE</a></b></div></body>";
			$nits_rec->Mailer   = "smtp";
			$nits_rec->Priority = '1';
			$nits_rec->Body = $body;
			$nits_rec->AltBody = "We have received your request for the non-faculty position at National Institute of Technology, Silchar. To verify your account http://recruitment.nits.ac.in/verify.php?vid=".$verify_id."&rid=".$reg_id." Click on the link.";
			$nits_rec->AddAddress($email);
	
			if(!$nits_rec->Send())
			{
				echo "There has been a mail error sending to " . $_POST['email'] . "<br/>Error info : ". $nits_rec->ErrorInfo;
			}
